...
async function select_server(sid){
    var server_info = server_list[sid];
    chrome.storage.local.set({mzk_select_server_info: server_info,mzk_server_id:sid}, function () {
        chrome.runtime.sendMessage({cmd:"reload_server_line"});
        start_loading();
        open_vpn();
    });
    await sleepms(1500);
